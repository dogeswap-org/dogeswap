...
describe("Pair", () => {
    ...
    describe("constructor", () => {
        it("cannot be used for tokens on different chains", () => {
            expect(() => new Pair(new CurrencyAmount(USDC, "100"), new CurrencyAmount(testWDC, "100"))).toThrow(
                "CHAIN_IDS",
            );
        });
    });
    ...
    describe("#chainId", () => {
        it("returns the token0 chainId", () => {
            expect(new Pair(new CurrencyAmount(USDC, "100"), new CurrencyAmount(DAI, "100")).chainId).toEqual(
                ChainId.MAINNET,
            );
            expect(new Pair(new CurrencyAmount(DAI, "100"), new CurrencyAmount(USDC, "100")).chainId).toEqual(
                ChainId.MAINNET,
            );
        });
    });
    describe("#involvesToken", () => {
        expect(new Pair(new CurrencyAmount(USDC, "100"), new CurrencyAmount(DAI, "100")).involvesToken(USDC)).toEqual(
            true,
        );
        expect(new Pair(new CurrencyAmount(USDC, "100"), new CurrencyAmount(DAI, "100")).involvesToken(DAI)).toEqual(
            true,
        );
        expect(
