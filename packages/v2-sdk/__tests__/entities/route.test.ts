...
describe("Route", () => {
    ...
    it("can have a token as both input and output", () => {
        const route = new Route([pair_0_wdc, pair_0_1, pair_1_wdc], testWWDOGE, testWWDOGE);
    ...
    it("supports ether input", () => {
        const route = new Route([pair_0_wdc], testWWDOGE, NativeToken.Instance);
    ...
    it("supports ether output", () => {
        const route = new Route([pair_0_wdc], testWWDOGE, token0, NativeToken.Instance);
        expect(route.pairs).toEqual([pair_0_wdc]);
        expect(route.input).toEqual(token0);
